...
const Config = /*@__PURE__*/Facet.define({
    combine: (c) => {
        ...
        for (let config of c) {
            ...
            const rest = __rest(config, ["create"]);
            configs.push(Object.assign(Object.assign({}, rest), { enabled: true }));
        }
...
const OverlayView = /*@__PURE__*/ViewPlugin.fromClass(class {
    ...
    onMouseDown(event) {
        ...
        const { clientY, target, pointerId } = event;
    ...
    onMouseMove(event) {
        ...
        this._dragStartY = event.clientY;
        const canvasHeight = this.view.scrollDOM.clientHeight;
...
const minimapClass = /*@__PURE__*/ViewPlugin.fromClass(class {
    ...
    render() {
        ...
        const scrollDOMHeight = this.view.scrollDOM.clientHeight;
        this.inner.style.minHeight = scrollDOMHeight + "px";
        this.canvas.height = scrollDOMHeight * Scale.PixelMultiplier;
        this.canvas.style.height = scrollDOMHeight + "px";
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            let gutter = gutters[i+1];
            if (gutter) {
                drawContext.offsetX += 2;
                drawLineGutter({[i+1]: gutter}, drawContext, i + 1);
                drawContext.offsetX += 4;
            } else {
                drawContext.offsetX += 6;
            }
